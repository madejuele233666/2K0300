...
- builder 不使用 lateral jump 阈值；近端已经存在时，远端 interval 只有在 BEV element raster 上与近端点的整数栅格连线不经过 black cell 时才可接回 leading path。
- index 0 没有 interval，则当前视觉 reference 无事实。
- 中间断点若没有通过 raster 连通验证，则断点之后全部 `present=false`、`source=kNone`。
- 通过 raster 连通验证补齐的中间点必须标记为 `source=kRasterConnected`；远端实际观测 interval 仍标记为 `source=kIntervalCenter`。
...
- candidate builder 必须构造从 index 0 开始的近端连续 `BEVReferencePath`；跨 gap 只能使用自己的视觉合同证明连通，不能把 unknown / image border / FOV boundary 当路径事实。
...
- 只加权到 `BEV_CONTROL_MODEL.LATERAL_ERROR_MAX_WEIGHTED_SAMPLE_INDEX`，更远 reference 点只保留为 evidence/debug。
- 权重按指数衰减，index 0 权重为 `1.0`，configured max index 权重为 `BEV_CONTROL_MODEL.LATERAL_ERROR_FAR_WEIGHT`。
- 不依赖 speed。
- 不看 source/mode。
- 不读 `PerceptionResult`。
...
- `BEVPathPointSource`：`kNone`、`kIntervalCenter`、`kRasterConnected`、`kHold`。
