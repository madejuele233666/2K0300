...
```text
camera frame
-> sparse BEV sample rows
-> row white intervals
-> current visual reference facts
...
- 不改 BEV 标定。
- 不改纵向尺度修正。
- runtime 默认不构建 dense BEV。
- runtime authority 是 sparse / virtual BEV row scan。
- dense BEV 只能通过显式 debug API 构建，例如 `BuildDebugDenseBevImage()`。
- dense BEV 不得被 reference、control、scene、fallback 回读。
...
- 元素识别后的路径策略必须再进入 reference facts / usability / lateral error / safety gate 链路。
...
- 只做 sparse BEV row scan、white intervals、current visual reference facts。
...
- 负责 current / hold / none selection。
