...
`ai-enforced-workflow` SHALL define verifier execution in terms of a platform-neutral runner contract rather than a hard-coded Windows PowerShell command. The contract MUST identify the required inputs, required outputs, retry semantics, and raw-envelope recovery behavior independently from the underlying shell implementation.
...
- **THEN** they refer to the verifier runner contract and its required parameters
...
The verifier runner contract MUST support at least Windows and Linux resolution paths that preserve equivalent behavior. Windows resolution MAY invoke a PowerShell helper, and Linux resolution MAY invoke a shell helper, but both implementations MUST honor the same prompt, raw report, normalized report, retry, and recovery contract.
...
### Requirement: Execution Evidence Captures Command And Verifier Profile
Runner execution evidence SHALL capture the resolved platform command for each
verification iteration. Verifier runtime profile SHOULD be recorded when a
checkpoint-specific override is used so checkpoints remain reproducible across
environments.

#### Scenario: Linux checkpoint records command and profile
- **WHEN** verification executes on Linux
- **THEN** execution evidence records the resolved Linux command
- **AND** execution evidence records verifier model/effort when an override is applied

#### Scenario: Windows checkpoint records command and profile
