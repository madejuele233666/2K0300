...
The accepted baseline SHALL continue to use `new/user/debug.sh tuning ...` as the project-owned host tuning workflow. It SHALL be able to connect to the runtime, send supported commands, plot left/right target-versus-measured speed curves with a minimal visualization, and save CSV evidence for repeated PID tuning runs. For steering tuning, that same accepted workflow MAY optionally listen on a separate steering media port, record image frames plus metadata, and align those artifacts with control-plane CSV and command logs.
