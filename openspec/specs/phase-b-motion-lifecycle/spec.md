...
The Phase B runtime SHALL define a project-owned motion lifecycle that is evaluated separately from process startup/shutdown and separately from the safety gate. The lifecycle MUST at minimum expose `DISARMED`, `START_REQUESTED`, `SPINUP`, `RUNNING`, `STOPPING`, and `FAIL_SAFE_LATCHED` phases.
