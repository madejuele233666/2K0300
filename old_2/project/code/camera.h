...
#define Width 160						//图像宽度
#define Height 120						//图像高度

#define START_LINE (Height-2)        //八邻域搜线起始行
#define NEW_LINE 80                 //八领域新种子行
#define END_LINE 20
#define zhidao_END 2                        //八邻域边界搜索停止行

#define HIGHEST_line_down 5

#define img_pixel(x,y)  ((int)(*(rgay_image+(x)*Width+(y))))

#define TRAIL8ALL_ARRAYLENGTH   400

#define adding_line_slope       0.98//限高20后实际值 0.93577   //0.6853        //斜率 实际值 0.68                   0.98
#define adding_line_intercept   61.6//限高20后实际值 18.5779     //20   //截距   实际值 55                  59.6
...
#define Grow_Judge_Corner 0.7

//********************************八邻域轨迹用到的定义********************************
enum Trail8AllSide {SideLeft, SideRight,SideLeftMid,SideRightMid};       //八邻域左右边界方向
typedef struct
{
    struct
    {
...
//第一个表示路障的检测状态（检测到或未检测到），第二个表示路障的类型（左路障或右路障），第三个表示路障检测的延迟距离（？）
typedef struct
{
...
enum StateofStart{run_start,start_over,run_stop,run_brushless_start};//启动标志位
typedef struct
{
...
extern int16_t mid_servo,show_mid_line;
...
extern int BLDC_PWM;
extern float circle_k;
extern int blok;
extern int farthest_line;
extern int zhidao_flag;
extern int circle_fast_find_left;
extern int circle_fast_find_right;
extern int maybe_circle_distance;
#endif /* CAMERA_H_ */
