...
void MPU6050_datainit(void)
{
    ...
    for(i = 100;i>0;i--)
    {
        ...
        if(imu660ra_gyro_z != 0 && imu660ra_gyro_y != 0 && imu660ra_gyro_x != 0)
        {
            num++;
            gx+=imu660ra_gyro_x;
            gy+=imu660ra_gyro_y;
...
void Prepare_Data(void)
{
    ...
    SortAver_FilterXYZ(&imu660ra_acc_x,&imu660ra_acc_y,&imu660ra_acc_z,&Acc_filt,12);//对加速度原始数据进行去极值滑动窗口滤波
    ...
    //陀螺仪AD值 转换成 弧度/秒
    Gyr_rad.X = (float) (imu660ra_gyro_x - gyro_x_init) * Gyro_Gr;
    Gyr_rad.Y = (float) (imu660ra_gyro_y - gyro_y_init) * Gyro_Gr;
    Gyr_rad.Z = (float) (imu660ra_gyro_z - gyro_z_init) * Gyro_Gr;
