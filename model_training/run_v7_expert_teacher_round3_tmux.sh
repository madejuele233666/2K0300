...
RUN_ID="${1:-v7_expert_teacher_round3_$(date +%Y%m%d_%H%M%S)}"
ROOT="experiments/${RUN_ID}"
TEACHER_DIR="${ROOT}/teacher_bundles"
STABLE_STAGE="${V7_STABLE_STAGE:-stage1_stable_negative_margin}"
RESCUE_STAGE="${V7_RESCUE_STAGE:-stage1_rescue_negative_margin}"
...
STABLE_SHARDS="${V7_STABLE_SHARDS:-4}"
RESCUE_SHARDS="${V7_RESCUE_SHARDS:-4}"
STABLE_LIMIT="${V7_STABLE_LIMIT:-32}"
RESCUE_LIMIT="${V7_RESCUE_LIMIT:-32}"
MAX_BOARD_US="${V7_MAX_BOARD_US:-12000}"
STABLE_SEEDS="${V7_STABLE_SEEDS:-20263601,20263603,20263605}"
RESCUE_SEEDS="${V7_RESCUE_SEEDS:-20263611,20263613,20263615}"
EPOCHS="${V7_EPOCHS:-260}"
PATIENCE="${V7_PATIENCE:-46}"
...
if [[ ! -f "${STABLE_CANDIDATES}" || ! -f "${RESCUE_CANDIDATES}" ]]; then
  venv/bin/python generate_v7_expert_teacher_round3_candidates.py \
    ...
    2>&1 | tee -a "${ROOT}/generate_expert_round3_candidates.log"
...
cat > "${ROOT}/launch_config.json" <<EOF
{
  ...
  "epochs": ${EPOCHS},
  "patience": ${PATIENCE},
  "note": "V7 round3 Expert-CTD with correct-teacher replay and explicit negative parent margin"
}
EOF
...
launch_stage "stable" "${STABLE_DIR}" "${STABLE_CANDIDATES}" "${STABLE_TEACHER_LABELS}" "${STABLE_SHARDS}" "${STABLE_LIMIT}" "${STABLE_SEEDS}" "v7stable3"
launch_stage "rescue" "${RESCUE_DIR}" "${RESCUE_CANDIDATES}" "${RESCUE_TEACHER_LABELS}" "${RESCUE_SHARDS}" "${RESCUE_LIMIT}" "${RESCUE_SEEDS}" "v7rescue3"

echo "v7_expert_teacher_round3 root=${ROOT} stable=${STABLE_DIR} rescue=${RESCUE_DIR} shards=$((STABLE_SHARDS + RESCUE_SHARDS))"
