#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "${SCRIPT_DIR}"

RUN_ID="${1:-v8_combo_compile_$(date +%Y%m%d_%H%M%S)}"
ROOT="experiments/${RUN_ID}"
mkdir -p "${ROOT}"
printf '%s\n' "${RUN_ID}" > experiments/v8_combo_compile_active_run.txt

RUN_TAG="$(printf '%s' "${RUN_ID}" | tr -c 'A-Za-z0-9_' '_' | cut -c1-34)"
OMP_THREADS="${OMP_THREADS:-2}"
TF_INTRA_THREADS="${TF_INTRA_THREADS:-2}"
TF_INTER_THREADS="${TF_INTER_THREADS:-1}"
GLOBAL_COMPILE_LOCK="${GLOBAL_COMPILE_LOCK:-experiments/v8_low_focus_parallel_20260521_0001/compile.lock}"
COMPILE_LOCK_MODE="${COMPILE_LOCK_MODE:-shared}"
COMPILE_TARGETS="${COMPILE_TARGETS:-p1_u6_combo_m4,p2_u6_combo_m8,p3_u6_combo_m4_sd,p4_d8_qanchor_combo_m4,p5_u10_rawlowedge_combo_m4,p6_u6_fullaug_m4}"

BASE_PARAMS="experiments/v8_parent_c2612_d4_multiteacher_20260520_0001/s2d_c2_6_12_d4_shadow_init_compile/best_parent_logits_memory_params.npz"
SOURCE_DECISION="experiments/v8_source_decision_margin_teacher_20260521_0002/d4base_c248pool_c248d4_d8q_d6sm_d7sm_t8/source_decision_margin_teacher.npz"
U6_RUN="experiments/v8_resource_parallel_20260521_0001/u6_wide_d20_identity_sgrank"
U10_RUN="experiments/v8_resource_parallel_20260521_0001/u10_wide_d20_rawlowedge_sgrank"
D8_QANCHOR_RUN="experiments/v8_parent_c2612_d8_multisource_qanchor_20260521_0001/s2d_c2_6_12_d8_shadow_init"

DYNAMIC_NORMAL_STRESS="rot90,rot180,rot270,mirror_lr,mirror_lr_rot90,mirror_lr_rot180,mirror_lr_rot270,cam_blur3a135,hblur5_noise_0p06,cam_blur5a135,cam_noise0p04,cam_blur5a45_noise0p04,cam_blur3a135_noise0p02,cam_blur3a0_noise0p02,diagblur5,cam_blur5a45,cam_bright0p04_contrast0p10,cam_noise0p02,vblur5,cam_contrast0p12,noise_0p10,diagblur5_noise_0p08,cam_blur2a0,cam_blur3a45,cam_bright0p06,cam_noise0p03,cam_blur3a90,noise_0p06,cam_blur3a45_noise0p02"
SYNTH_EXTRA_STRESS="cam_noise0p06,cam_noise0p08,cam_noise0p10,cam_blur3a0,cam_blur5a0,cam_blur5a90,cam_blur7a45,cam_blur7a135,cam_blur5a135_noise0p04,cam_blur7a45_noise0p04,cam_brightp0p04,cam_brightp0p08,cam_brightp0p12,cam_brightm0p04,cam_brightm0p08,cam_brightm0p12,cam_contrastp0p10,cam_contrastp0p20,cam_contrastm0p10,cam_contrastm0p20,cam_shiftu1,cam_shiftd1,cam_shiftl1,cam_shiftr1,cam_shiftul1,cam_shiftdr1,cam_shiftu2,cam_shiftd2,cam_shiftl2,cam_shiftr2,cam_shiftu1_noise0p04,cam_shiftl1_noise0p04"
COMBO_STRESS="cam_rot270_shiftl2_blur3a45,cam_rot270_shiftd1_blur3a45,cam_rot270_mirror_lr_shiftl2_blur3a45,cam_rot270_mirror_lr_shiftl1_blur3a45,cam_rot180_mirror_lr_shiftl1_blur3a45,cam_rot90_shiftl1_blur3a45,cam_rot90_shiftd1_blur3a135_noise0p02,cam_rot90_mirror_lr_shiftl1_blur3a45,cam_rot270_contrastm0p10_blur3a45,cam_rot270_mirror_lr_contrastm0p10_blur3a45,cam_rot270_brightm0p08_blur3a45,cam_rot90_brightp0p08_blur3a135"
COMBO_COMPILE_STRESS="${DYNAMIC_NORMAL_STRESS},${COMBO_STRESS}"
FULL_AUG_COMPILE_STRESS="${DYNAMIC_NORMAL_STRESS},${SYNTH_EXTRA_STRESS},${COMBO_STRESS}"

for path in "${BASE_PARAMS}" "${SOURCE_DECISION}" "${U6_RUN}/parent_int8.tflite" "${U10_RUN}/parent_int8.tflite" "${D8_QANCHOR_RUN}/parent_int8.tflite"; do
  if [[ ! -e "${path}" ]]; then
    echo "missing required path: ${path}" >&2
    exit 1
  fi
done

launch_session() {
  local session="$1"
  local cmd="$2"
  if tmux has-session -t "${session}" 2>/dev/null; then
    echo "exists ${session}"
    return 0
  fi
  printf -v quoted_cmd '%q' "${cmd}"
  tmux new-session -d -s "${session}" "bash -lc ${quoted_cmd}"
  echo "launched ${session}"
}

launch_compile_eval() {
  local name="$1"
  local parent_run="$2"
  local stress_list="$3"
  local target_margin="$4"
  local max_iters="$5"
  local extra_compile_flags="$6"
  local compile_dir="${ROOT}/${name}_compile"
  local stress_dir="${ROOT}/${name}_highstress_canonical"
  local session="v8cc_${RUN_TAG}_${name}"
  local cmd
  local lock_prefix=""
  local lock_cmd=""
  local lock_suffix=""
  local lock_redirect=""
  if [[ "${COMPILE_LOCK_MODE}" == "shared" ]]; then
    lock_prefix="("
    lock_cmd="flock 9"
    lock_suffix=")"
    lock_redirect=" 9>${GLOBAL_COMPILE_LOCK}"
  elif [[ "${COMPILE_LOCK_MODE}" == "none" ]]; then
    lock_prefix=""
    lock_suffix=""
    lock_redirect=""
  else
    echo "unknown COMPILE_LOCK_MODE: ${COMPILE_LOCK_MODE}" >&2
    exit 1
  fi
  cmd=$(cat <<EOF
set -euo pipefail
cd ${SCRIPT_DIR}
mkdir -p ${compile_dir} ${stress_dir}
${lock_prefix}
  ${lock_cmd}
  echo compile_start ${compile_dir}
  CUDA_VISIBLE_DEVICES="" OMP_NUM_THREADS=${OMP_THREADS} TF_NUM_INTRAOP_THREADS=${TF_INTRA_THREADS} TF_NUM_INTEROP_THREADS=${TF_INTER_THREADS} venv/bin/python compile_v8_parent_logits_memory.py \
    --parent-run-dir ${parent_run} \
    --dataset-dir dataset \
    --output-dir ${compile_dir} \
    --stress ${stress_list} \
    --feature-sources int8_tflite,float_tflite \
    --prototype-subsets clean,clean_rotmirror,all \
    --residual-bases clean,clean_rotmirror \
    --residual-target-int8-margin ${target_margin} \
    --max-residual-iterations ${max_iters} \
    ${extra_compile_flags} 2>&1 | tee -a ${compile_dir}/run.log
  echo highstress_start ${stress_dir}
  CUDA_VISIBLE_DEVICES="" OMP_NUM_THREADS=${OMP_THREADS} TF_NUM_INTRAOP_THREADS=${TF_INTRA_THREADS} TF_NUM_INTEROP_THREADS=${TF_INTER_THREADS} venv/bin/python stress_test_v8_low_margin.py \
    --tflite ${parent_run}/parent_int8.tflite \
    --params-npz ${compile_dir}/best_parent_logits_memory_params.npz \
    --selection-params-npz ${BASE_PARAMS} \
    --dataset-dir dataset \
    --output-dir ${stress_dir} \
    --perturbs all \
    --low-margin-threshold 8 \
    --control-margin-min 128 \
    --batch-size 512 2>&1 | tee -a ${stress_dir}/run.log
  echo compile_eval_done ${stress_dir}
${lock_suffix}${lock_redirect}
EOF
)
  launch_session "${session}" "${cmd}"
}

should_launch_target() {
  local target="$1"
  [[ ",${COMPILE_TARGETS}," == *",${target},"* ]]
}

cat > "${ROOT}/launch_config.json" <<EOF
{
  "run_id": "${RUN_ID}",
  "purpose": "Compile-only normal-generated composite residual coverage; no high-pressure samples in training, teacher, prototype selection, or pruning.",
  "high_pressure_usage": "evaluation_only",
  "combo_compile_stress": "${COMBO_COMPILE_STRESS}",
  "full_aug_compile_stress": "${FULL_AUG_COMPILE_STRESS}",
  "selection_params": "${BASE_PARAMS}",
  "global_compile_lock": "${GLOBAL_COMPILE_LOCK}",
  "compile_lock_mode": "${COMPILE_LOCK_MODE}",
  "compile_targets": "${COMPILE_TARGETS}"
}
EOF

should_launch_target "p1_u6_combo_m4" && launch_compile_eval "p1_u6_combo_m4" "${U6_RUN}" "${COMBO_COMPILE_STRESS}" 4 20 ""
should_launch_target "p2_u6_combo_m8" && launch_compile_eval "p2_u6_combo_m8" "${U6_RUN}" "${COMBO_COMPILE_STRESS}" 8 24 ""
should_launch_target "p3_u6_combo_m4_sd" && launch_compile_eval "p3_u6_combo_m4_sd" "${U6_RUN}" "${COMBO_COMPILE_STRESS}" 4 20 "--source-decision-teacher-npz ${SOURCE_DECISION} --source-decision-compiler-margin 8"
should_launch_target "p4_d8_qanchor_combo_m4" && launch_compile_eval "p4_d8_qanchor_combo_m4" "${D8_QANCHOR_RUN}" "${COMBO_COMPILE_STRESS}" 4 20 ""
should_launch_target "p5_u10_rawlowedge_combo_m4" && launch_compile_eval "p5_u10_rawlowedge_combo_m4" "${U10_RUN}" "${COMBO_COMPILE_STRESS}" 4 20 ""
should_launch_target "p6_u6_fullaug_m4" && launch_compile_eval "p6_u6_fullaug_m4" "${U6_RUN}" "${FULL_AUG_COMPILE_STRESS}" 4 24 ""

echo "root=${ROOT}"
echo "sessions:"
tmux ls | grep -E "v8cc_${RUN_TAG}" | sort || true
