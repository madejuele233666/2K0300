...
RUN_ID="${1:-v7_expert_teacher_round2_$(date +%Y%m%d_%H%M%S)}"
ROOT="experiments/${RUN_ID}"
TEACHER_DIR="${ROOT}/teacher_bundles"
STABLE_STAGE="${V7_STABLE_STAGE:-stage1_stable_isolated}"
RESCUE_STAGE="${V7_RESCUE_STAGE:-stage1_rescue_isolated}"
...
STABLE_SEEDS="${V7_STABLE_SEEDS:-20263501,20263503,20263505}"
RESCUE_SEEDS="${V7_RESCUE_SEEDS:-20263511,20263513,20263515}"
...
if [[ ! -f "${STABLE_TEACHER_LABELS}" || ! -f "${RESCUE_TEACHER_LABELS}" ]]; then
  venv/bin/python build_v7_expert_teacher_round2_labels.py \
    --dataset-dir dataset \
    --old-tflite "${OLD_TFLITE}" \
    --rescue-tflite "${RESCUE_TFLITE}" \
    --output-dir "${TEACHER_DIR}" \
    --temperature 1.0 \
    2>&1 | tee -a "${ROOT}/build_expert_teacher_round2_labels.log"
fi

if [[ ! -f "${STABLE_CANDIDATES}" || ! -f "${RESCUE_CANDIDATES}" ]]; then
  venv/bin/python generate_v7_expert_teacher_round2_candidates.py \
    ...
    2>&1 | tee -a "${ROOT}/generate_expert_round2_candidates.log"
...
cat > "${ROOT}/launch_config.json" <<EOF
{
  ...
  "epochs": ${EPOCHS},
  "patience": ${PATIENCE},
  "note": "V7 round2 expert-preserving training with isolated stable/rescue responsibilities"
}
EOF
...
launch_stage "stable" "${STABLE_DIR}" "${STABLE_CANDIDATES}" "${STABLE_TEACHER_LABELS}" "${STABLE_SHARDS}" "${STABLE_LIMIT}" "${STABLE_SEEDS}" "v7stable2"
launch_stage "rescue" "${RESCUE_DIR}" "${RESCUE_CANDIDATES}" "${RESCUE_TEACHER_LABELS}" "${RESCUE_SHARDS}" "${RESCUE_LIMIT}" "${RESCUE_SEEDS}" "v7rescue2"

echo "v7_expert_teacher_round2 root=${ROOT} stable=${STABLE_DIR} rescue=${RESCUE_DIR} shards=$((STABLE_SHARDS + RESCUE_SHARDS))"
