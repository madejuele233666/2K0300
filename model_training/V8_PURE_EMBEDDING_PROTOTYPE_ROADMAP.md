...
2026-05-21 true source-specific conditional table 边界：新增 `evaluate_v8_true_source_conditional_table.py`，不训练新模型，只把 C248 under2 五源的真实 stress rows 和各自 normal params 组合成 source-specific conditional-table 诊断；canonical 高压仍只作最终评估。本次 `v8_true_source_conditional_table_20260521_0001/c248_under2_boundary` 中五个 source 单独 normal replay 均为 0 wrong，单 source 保守总估时分别约 `1788-2000us`，因此若存在“免费/极轻量”source selector，selected-source 路径本身仍可卡在 under2；但同时运行五个 source 再选需要约 `9747us`，超过 8ms，不是部署路径。真实 source 表的 evaluation-only oracle 仍为 `1327/24190`、low/control `9.71% / 1.26%`，而无标签 `max_stress_margin` 只有 `4288/24190`、`28.47% / 6.99%`，`margin_sum` 为 `4366/24190`、`28.63% / 7.47%`。该结果把边界拆清楚：source-specific geometry 本身足以覆盖 `<10%` low，但读取全部 source 输出再路由超预算，且简单 margin 路由远离 oracle。下一步不能回到 source block / collapsed residual，而应专攻低成本 source-orbit selector：要么让一个 under2 strict-op encoder 直接产生可分的 source/orbit side code，要么做 base D4 replay + 极小二级 selector/table，但二级内容必须引用真实 source-specific 表或等价独立子空间。
