...
RUN_ID="${1:-$(cat experiments/v6_active_run.txt 2>/dev/null || echo v6_parent_primary_fine_$(date +%Y%m%d_%H%M%S))}"
ROOT="experiments/${RUN_ID}"
STAGE_NAME="${V6_STAGE_NAME:-stage2_fine}"
OUT_BASE="${ROOT}/${STAGE_NAME}"
STAGE1_DIR="${V6_STAGE1_DIR:-${ROOT}/stage1_coarse}"
CANDIDATES_JSON="${OUT_BASE}/v6_fine_candidates.json"
RUN_TAG="$(printf '%s' "${RUN_ID}" | tr -c 'A-Za-z0-9_' '_' | cut -c1-44)"

SHARDS="${V6_SHARDS:-16}"
LIMIT="${V6_LIMIT:-480}"
SEEDS="${V6_SEEDS:-20262911}"
EPOCHS="${V6_EPOCHS:-190}"
PATIENCE="${V6_PATIENCE:-30}"
CALIBRATION_LIMIT="${CALIBRATION_LIMIT:-192}"
MAX_BOARD_US="${V6_MAX_BOARD_US:-11000}"
...
if [[ ! -d "${STAGE1_DIR}" ]]; then
  echo "missing stage1 dir: ${STAGE1_DIR}" >&2
  exit 2
fi
...
if [[ ! -f "${CANDIDATES_JSON}" ]]; then
  venv/bin/python generate_v6_parent_primary_fine_candidates.py \
...
cat > "${OUT_BASE}/launch_config.json" <<EOF
{
  ...
  "note": "stage2 fine scan centered on V6 coarse pareto families"
}
EOF

for i in $(seq 0 $((SHARDS - 1))); do
  out_dir="${OUT_BASE}/shard_${i}"
  session="v6pf_${RUN_TAG}_${STAGE_NAME}_${i}"
  mkdir -p "${out_dir}"
...
echo "v6_parent_primary_fine root=${OUT_BASE} candidates=${CANDIDATES_JSON} shards=${SHARDS} limit=${LIMIT} teacher=${TEACHER_TFLITE}"
