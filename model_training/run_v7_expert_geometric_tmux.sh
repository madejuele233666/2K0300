...
RUN_ID="${1:-v7_expert_geometric_$(date +%Y%m%d_%H%M%S)}"
ROOT="experiments/${RUN_ID}"
TEACHER_DIR="${ROOT}/teacher_bundles"
STABLE_STAGE="${V7_STABLE_STAGE:-stage2_stable_geometric_consistency}"
RESCUE_STAGE="${V7_RESCUE_STAGE:-stage2_rescue_geometric_consistency}"
STABLE_DIR="${ROOT}/${STABLE_STAGE}"
RESCUE_DIR="${ROOT}/${RESCUE_STAGE}"
CANDIDATE_DIR="${ROOT}/candidate_sets"
STABLE_CANDIDATES="${STABLE_DIR}/stable_geometric_candidates.json"
RESCUE_CANDIDATES="${RESCUE_DIR}/rescue_geometric_candidates.json"
...
STABLE_LIMIT="${V7_STABLE_LIMIT:-96}"
RESCUE_LIMIT="${V7_RESCUE_LIMIT:-96}"
BASE_STABLE_LIMIT="${V7_BASE_STABLE_LIMIT:-24}"
BASE_RESCUE_LIMIT="${V7_BASE_RESCUE_LIMIT:-24}"
MAX_BOARD_US="${V7_MAX_BOARD_US:-12000}"
STABLE_SEEDS="${V7_STABLE_SEEDS:-20263701,20263703}"
RESCUE_SEEDS="${V7_RESCUE_SEEDS:-20263711,20263713}"
EPOCHS="${V7_EPOCHS:-280}"
PATIENCE="${V7_PATIENCE:-52}"
...
mkdir -p "${ROOT}" "${TEACHER_DIR}" "${STABLE_DIR}" "${RESCUE_DIR}" "${CANDIDATE_DIR}"
...
if [[ ! -f "${STABLE_TEACHER_LABELS}" || ! -f "${RESCUE_TEACHER_LABELS}" ]]; then
  venv/bin/python build_v7_expert_teacher_round3_labels.py \
    --dataset-dir dataset \
    --old-tflite "${OLD_TFLITE}" \
    --rescue-tflite "${RESCUE_TFLITE}" \
    --output-dir "${TEACHER_DIR}" \
    --temperature 1.0 \
    2>&1 | tee -a "${ROOT}/build_expert_teacher_round3_labels.log"
fi

if [[ ! -f "${STABLE_CANDIDATES}" || ! -f "${RESCUE_CANDIDATES}" ]]; then
  venv/bin/python generate_v7_expert_geometric_candidates.py \
    --parent-candidates "${PARENT_CANDIDATES}" \
    --ctd-candidates "${CTD_CANDIDATES}" \
    --output-dir "${CANDIDATE_DIR}" \
    --stable-limit "${STABLE_LIMIT}" \
    --rescue-limit "${RESCUE_LIMIT}" \
    --base-stable-limit "${BASE_STABLE_LIMIT}" \
    --base-rescue-limit "${BASE_RESCUE_LIMIT}" \
    --max-board-us "${MAX_BOARD_US}" \
    2>&1 | tee -a "${ROOT}/generate_expert_geometric_candidates.log"
  cp "${CANDIDATE_DIR}/stable_geometric_candidates.json" "${STABLE_CANDIDATES}"
  cp "${CANDIDATE_DIR}/rescue_geometric_candidates.json" "${RESCUE_CANDIDATES}"
fi
...
cat > "${ROOT}/launch_config.json" <<EOF
{
  ...
  "note": "V7 strict expert training: old/CTD teacher, negative margin, 8-view parent consistency, rotmirror validation, balanced_rotmirror int8 calibration"
}
EOF
...
launch_stage "stable" "${STABLE_DIR}" "${STABLE_CANDIDATES}" "${STABLE_TEACHER_LABELS}" "${STABLE_SHARDS}" "${STABLE_LIMIT}" "${STABLE_SEEDS}" "v7geost"
launch_stage "rescue" "${RESCUE_DIR}" "${RESCUE_CANDIDATES}" "${RESCUE_TEACHER_LABELS}" "${RESCUE_SHARDS}" "${RESCUE_LIMIT}" "${RESCUE_SEEDS}" "v7geors"

echo "v7_expert_geometric root=${ROOT} stable=${STABLE_DIR} rescue=${RESCUE_DIR} shards=$((STABLE_SHARDS + RESCUE_SHARDS))"
