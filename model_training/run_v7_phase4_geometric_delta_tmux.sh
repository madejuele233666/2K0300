...
RUN_ID="${1:-v7_phase4_geometric_delta_$(date +%Y%m%d_%H%M%S)}"
ROOT="experiments/${RUN_ID}"
STAGE="${V7_PHASE4_STAGE:-stage1_geo_hardlabel_adapter_router}"
...
if [[ -f "${DEFAULT_PHASE3_CACHE}" ]]; then
  FEATURE_CACHE="${V7_PHASE4_FEATURE_CACHE:-${DEFAULT_PHASE3_CACHE}}"
else
  FEATURE_CACHE="${V7_PHASE4_FEATURE_CACHE:-${CACHE_DIR}/old_rescue_clean_all_stress_gap_logits.npz}"
fi
...
SHARDS="${V7_PHASE4_SHARDS:-16}"
MAX_ADAPTER_COMBOS="${V7_PHASE4_MAX_ADAPTER_COMBOS:-1600}"
ROUTER_TOP_ADAPTERS="${V7_PHASE4_ROUTER_TOP_ADAPTERS:-160}"
MAX_THRESHOLDS="${V7_PHASE4_MAX_THRESHOLDS:-56}"
MAX_GATES_PER_ADAPTER="${V7_PHASE4_MAX_GATES_PER_ADAPTER:-520}"
MAX_KEPT_ROWS="${V7_PHASE4_MAX_KEPT_ROWS:-30000}"
OMP_THREADS="${OMP_THREADS:-1}"
TF_INTRA_THREADS="${TF_INTRA_THREADS:-1}"
TF_INTER_THREADS="${TF_INTER_THREADS:-1}"

VIEW_PROFILES="${V7_PHASE4_VIEW_PROFILES:-rotmirror,rotmirror_noise,rotmirror_blur_noise,rotmirror_camera_full,worst_phase2,all_stress}"
POSITIVE_PROFILES="${V7_PHASE4_POSITIVE_PROFILES:-old_wrong_rotmirror,old_wrong_rotmirror_plus_rescue,old_wrong_all_views,old_wrong_all_plus_rescue,stress_recovery_allow_stable,hybrid_clean_stress}"
WEIGHT_PROFILES="${V7_PHASE4_WEIGHT_PROFILES:-strict_geo_lock,rotmirror_dro,group_dro,preserve_locked,rescue_heavy,camera_guard}"
TARGET_MODES="${V7_PHASE4_TARGET_MODES:-margin_target,hybrid_ctd_margin,ctd_delta}"
L2_VALUES="${V7_PHASE4_L2:-0.0001,0.001,0.01,0.1,1,10,30}"
GATE_L2_VALUES="${V7_PHASE4_GATE_L2:-0.0001,0.001,0.01,0.1,1}"
RANK_VALUES="${V7_PHASE4_RANK:-full,1,2,3,4,6}"
MASK_PERCENTILES="${V7_PHASE4_MASK_PERCENTILE:-0,50,70,85,92}"
ALPHA_VALUES="${V7_PHASE4_ALPHA:-0.75,1.0,1.25,1.5,2.0,2.5}"
MARGIN_VALUES="${V7_PHASE4_MARGIN:-1,2,3,4,6,8,10}"
BLEND_VALUES="${V7_PHASE4_BLEND:-0.0,0.25,0.5,0.75}"
...
cat > "${ROOT}/launch_config.json" <<EOF
{
  ...
  "blend": "${BLEND_VALUES}",
  "note": "Phase4 geometric delta-router search: old stable frozen, old-wrong hard-label positives, clean and rot_mirror scored as hard gates, stress used for ranking."
}
EOF
...
echo "v7_phase4_geometric_delta root=${ROOT} stage=${STAGE_DIR} shards=${SHARDS}"
