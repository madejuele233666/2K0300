...
RUN_ID="${1:-v8_standalone_source_gate_current_$(date +%Y%m%d_%H%M%S)}"
ROOT="experiments/${RUN_ID}"
mkdir -p "${ROOT}"
printf '%s\n' "${RUN_ID}" > experiments/v8_standalone_source_gate_current_active_run.txt
...
STANDALONE_TARGETS="${STANDALONE_TARGETS:-sg4_diverse_l4,sg6_synth_top,sg8_no_tables}"
...
TRAIN_STRESS="${TRAIN_STRESS:-cam_shiftu1,cam_shiftd1,cam_shiftl1,cam_shiftr1,cam_shiftu2,cam_shiftd2,cam_shiftl2,cam_shiftr2,cam_shiftu1_noise0p04,cam_shiftl1_noise0p04,cam_blur3a45,cam_blur3a135,cam_blur5a45,cam_blur5a135,cam_blur7a45,cam_blur7a135,cam_blur5a45_noise0p04,cam_blur5a135_noise0p04,cam_blur7a45_noise0p04,cam_noise0p04,cam_noise0p08,cam_noise0p10,cam_contrastm0p10,cam_contrastm0p20,cam_brightm0p08,cam_brightm0p12}"
FILTERS="${FILTERS:-2,6,12}"
EPOCHS="${EPOCHS:-180}"
LEARNING_RATE="${LEARNING_RATE:-0.0025}"
BATCH_SIZE="${BATCH_SIZE:-256}"
TEACHER_SCORE_MODE="${TEACHER_SCORE_MODE:-log}"
TEACHER_NORMAL_MARGIN_MAX="${TEACHER_NORMAL_MARGIN_MAX:-1000000000}"
...
for path in \
  ...
  "${U6_PARAMS}" "${U7_PARAMS}" "${U8_PARAMS}" "${C5_PARAMS}" "${L1_PARAMS}" "${L2_PARAMS}" "${L3_PARAMS}" "${L4_PARAMS}" \
  "${U6_STRESS}" "${U7_STRESS}" "${U8_STRESS}" "${C5_STRESS}" "${L1_STRESS}" "${L2_STRESS}" "${L3_STRESS}" "${L4_STRESS}"; do
  if [[ ! -e "${path}" ]]; then
...
launch_gate() {
  ...
  local teacher_dir="${out_dir}/teacher"
  local session="v8sgd_${RUN_TAG}_${name}"
  local cmd
  cmd=$(cat <<EOF
set -euo pipefail
cd ${SCRIPT_DIR}
mkdir -p ${teacher_dir} ${out_dir}
echo standalone_source_gate_start ${out_dir}
CUDA_VISIBLE_DEVICES="" OMP_NUM_THREADS=${OMP_THREADS} TF_NUM_INTRAOP_THREADS=${TF_INTRA_THREADS} TF_NUM_INTEROP_THREADS=${TF_INTER_THREADS} venv/bin/python build_v8_synthetic_source_gate_teacher.py \
  ${tflite_args} \
  ${params_args} \
  --base-params ${BASE_PARAMS} \
  --highpressure-stress ${U6_STRESS} \
  --dataset-dir dataset \
  --output-dir ${teacher_dir} \
  --perturbs ${PERTURBS} \
  --normal-margin-max ${TEACHER_NORMAL_MARGIN_MAX} \
  --max-base-rows ${max_rows} \
  --score-mode ${TEACHER_SCORE_MODE} \
  --temperature 0.07 \
  --label-smoothing 0.02 \
  --gap-weight 6.0 \
  --batch-size 512 2>&1 | tee -a ${out_dir}/run.log
TF_FORCE_GPU_ALLOW_GROWTH=true OMP_NUM_THREADS=${OMP_THREADS} TF_NUM_INTRAOP_THREADS=${TF_INTRA_THREADS} TF_NUM_INTEROP_THREADS=${TF_INTER_THREADS} ./run_gpu.sh venv/bin/python train_v8_standalone_source_gate.py \
  --dataset-dir dataset \
  --source-gate-teacher-npz ${teacher_dir}/source_gate_teacher.npz \
  ${stress_args} \
  --output-dir ${out_dir} \
  --stress ${TRAIN_STRESS} \
  --filters ${FILTERS} \
  ...
  --validation-mod 5 \
  --class-balance inverse_label \
  --max-class-balance-weight 8 \
  --seed 2026053001 2>&1 | tee -a ${out_dir}/run.log
EOF
)
...
G4_STRESS_ARGS="\
  --source u6=${U6_STRESS} \
  --source c5_raw=${C5_STRESS} \
  --source l1_sg=${L1_STRESS} \
  --source l4_d24=${L4_STRESS}"
...
G8_STRESS_ARGS="\
  ...
  --source l1_sg=${L1_STRESS} \
  --source l2_dyn=${L2_STRESS} \
  --source l3_sd=${L3_STRESS} \
  --source l4_d24=${L4_STRESS}"

cat > "${ROOT}/launch_config.json" <<EOF
{
  "run_id": "${RUN_ID}",
  "purpose": "Train deployable standalone source-gate networks from normal/synthetic source-winner teachers; high-pressure remains evaluation-only.",
  ...
  "teacher_normal_margin_max": ${TEACHER_NORMAL_MARGIN_MAX},
  "filters": "${FILTERS}",
  "epochs": ${EPOCHS},
...
should_launch_target "sg4_diverse_l4" && launch_gate "sg4_diverse_l4" 700 "${G4_TFLITE_ARGS}" "${G4_PARAMS_ARGS}" "${G4_STRESS_ARGS}"
should_launch_target "sg6_synth_top" && launch_gate "sg6_synth_top" 600 "${G6_TFLITE_ARGS}" "${G6_PARAMS_ARGS}" "${G6_STRESS_ARGS}"
should_launch_target "sg8_no_tables" && launch_gate "sg8_no_tables" 520 "${G8_TFLITE_ARGS}" "${G8_PARAMS_ARGS}" "${G8_STRESS_ARGS}"

echo "root=${ROOT}"
echo "sessions:"
tmux ls | grep -E "v8sgd_${RUN_TAG}" | sort || true
