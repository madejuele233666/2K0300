...
def build_stable_bundle(
    ...
    parent_weight[masks["hard"] & old_zone] = np.maximum(
        parent_weight[masks["hard"] & old_zone],
        args.stable_hard_parent_weight,
    )
    parent_weight[masks["c4"] & old_zone] = np.maximum(
        parent_weight[masks["c4"] & old_zone],
        args.stable_c4_parent_weight,
    )
...
def build_rescue_bundle(
    ...
    soft[masks["hard"] & masks["rescue"]] = blend_with_hard(
        rescue_soft[masks["hard"] & masks["rescue"]],
        y_parent[masks["hard"] & masks["rescue"]],
        args.rescue_hard_clean_alpha,
    )
    # Preserve samples are explicit anti-CTD constraints: hard human parent, no CTD KD.
...
def write_bundle(
    ...
    with summary_csv.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=["path", "visual", "parent", "group", "old_pred", "rescue_pred", "teacher_weight", "parent_weight"],
        )
        writer.writeheader()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build V7 round2 isolated expert teacher bundles.")
    ...
    parser.add_argument("--stable-teacher-weight", type=float, default=1.00)
    parser.add_argument("--preserve-teacher-weight", type=float, default=1.40)
    parser.add_argument("--stable-hard-teacher-weight", type=float, default=1.60)
    parser.add_argument("--stable-parent-weight", type=float, default=0.35)
    parser.add_argument("--preserve-parent-weight", type=float, default=1.20)
    parser.add_argument("--stable-rescue-parent-weight", type=float, default=0.0)
    parser.add_argument("--stable-hard-parent-weight", type=float, default=1.00)
    parser.add_argument("--stable-c4-parent-weight", type=float, default=1.00)
    parser.add_argument("--stable-hard-alpha", type=float, default=0.08)
    parser.add_argument("--stable-hard-clean-alpha", type=float, default=0.18)

    parser.add_argument("--rescue-teacher-weight", type=float, default=1.30)
    parser.add_argument("--rescue-stable-teacher-weight", type=float, default=0.05)
    parser.add_argument("--rescue-hard-teacher-weight", type=float, default=1.50)
    parser.add_argument("--rescue-c4-teacher-weight", type=float, default=1.50)
    parser.add_argument("--rescue-stable-parent-weight", type=float, default=0.20)
    parser.add_argument("--rescue-preserve-parent-weight", type=float, default=5.00)
    parser.add_argument("--rescue-parent-weight", type=float, default=8.00)
    parser.add_argument("--rescue-hard-parent-weight", type=float, default=5.00)
    parser.add_argument("--rescue-c4-parent-weight", type=float, default=6.00)
    parser.add_argument("--rescue-circuit-parent-weight", type=float, default=8.00)
    parser.add_argument("--rescue-hard-alpha", type=float, default=0.35)
    parser.add_argument("--rescue-hard-clean-alpha", type=float, default=0.45)
    parser.add_argument("--rescue-stable-hard-alpha", type=float, default=0.15)
    ...
    stable_soft, stable_teacher_weight, stable_parent_weight = build_stable_bundle(y_parent, old_soft, masks, args)
    rescue_soft_out, rescue_teacher_weight, rescue_parent_weight = build_rescue_bundle(y_parent, old_soft, rescue_soft, masks, args)
    base_meta = {
        "builder": "build_v7_expert_teacher_round2_labels.py",
        "old_tflite": str(args.old_tflite),
        "rescue_tflite": str(args.rescue_tflite),
        "temperature": args.temperature,
        "counts": counts,
        "policy": "stable preserves old zones and ignores rescue; rescue focuses rescue/hard/c4 and treats preserve as anti-CTD hard constraints",
    }
    write_bundle(
        ...
        {**base_meta, "role": "stable_isolated"},
    )
    write_bundle(
        ...
        {**base_meta, "role": "rescue_isolated"},
    )
    print(json.dumps({"counts": counts, "stable_teacher_active": int(np.sum(stable_teacher_weight > 0)), "rescue_teacher_active": int(np.sum(rescue_teacher_weight > 0))}, indent=2, ensure_ascii=False))
