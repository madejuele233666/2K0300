...
- Historical references below describe the earlier wrong-baseline port attempt unless a later section explicitly rebinds them to the true baseline.

## Why

`old/` contains working control and perception logic, but it is tied to the TC264 bare-metal library, interrupt model, and C driver APIs. The accepted target environment is `true_LS2K0300_Library/Seekfree_LS2K0300_Opensource_Library`, a Linux/LoongArch application stack with different device abstractions, so direct copy-paste would mix platform code and algorithm code in a way that is hard to build, validate, and extend.
...
- Affected source areas: `old/user/`, `old/code/`, `old/libraries/` (reference only), `true_LS2K0300_Library/Seekfree_LS2K0300_Opensource_Library/project/`, and `new/`.
