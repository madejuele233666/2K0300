...
Expansion observer 当前还保留一组私有阈值。这些值不是 runtime 参数，不能由 `param_store` 复制一套独立合法性标准；如果未来要公开，必须先抽出 CircleV2 自己的 normalize/helper，再由调用方消费同一份语义。

| 内部值 | 当前值 | 语义 |
|---|---:|---|
| `kRatioDenominatorFloor` | `1.0e-4` | 比率计算的分母下限，避免边界 reach 接近 0 时增长率爆炸；同时给部分阈值提供极小正下限。 |
| `kOpeningSustainRows` | `2` | 开口增长必须在后续 2 行持续成立，不接受单行尖峰；因此真正的 opening evidence 仍至少需要 3 行。 |
| `min_support_rows` | `1` | 只表示有观测行即可进入 CircleV2 expansion 评估；线性拟合仍通过 `max(2, min_support_rows)` 保持至少 2 点。 |
| `min_sampleable_per_row` | `16` | 单行至少需要 16 个可采样横向 cell，才允许进入边界 trace。 |
| `center_sample_forward_gap_max_m` | `0.10m` | 使用普通道路中心线选择 road-connected interval 时，中心点 forward 与当前 row 的最大允许差距。 |
| `open_expansion_min_m` | `0.05m` | 一侧边界 reach 的绝对增长下限。 |
| `opening_expansion_ratio_min` | `0.10` | 一侧边界 reach 的相对增长下限；与 `open_expansion_min_m` 同时满足才算 opening。 |
| `opposite_straight_drift_max_m` | `0.06m` | 对侧边界线性拟合 RMSE 上限，超过后不视为可靠直边。 |
| `opposite_shrink_ratio_min` | `0.10` | 对侧边界向内收缩的相对比例门槛；超过后按 shrink 处理，不再当作可靠直边。 |
| `present_confidence_min` | `0.65` | 最终方向检测置信度门槛。 |

方向检测置信度当前由 `0.80 * open_score + 0.10 * opposite_fit_confidence + 0.10 * support_score` 组成。该权重语义是以开口增长为主，对侧直线和行数支持只做辅助。对侧直线拟合会对 squared error 排序后取较好的约 90% 计算 RMSE，用来降低单个坏点对直线判断的破坏。
...
CIRCLE_V2_ENTRY_BOTTOM_ROW_COUNT:
  Approach entry gate 使用的下部 ROI 行数，配置默认 3，合法范围 >= 1。
