...
private:
    ...
    void HandleInboundMessages(const std::vector<transport::AssistantInboundMessage>& inbound_messages,
                               RuntimeState& state,
                               port::DiagnosticSink& diagnostics,
                               uint64_t now_ms);
    /// 处理单条辅助命令（调参模式、转向抑制、目标速度、启动/停止）
    void HandleCommand(const transport::AssistantCommand& command,
                       RuntimeState& state,
                       port::DiagnosticSink& diagnostics,
                       uint64_t now_ms);
    ...
    transport::AssistantLink link_{};                ///< 辅助链路层实例
