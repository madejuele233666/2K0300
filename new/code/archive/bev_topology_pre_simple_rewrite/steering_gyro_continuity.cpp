...
GyroContinuityConstraint ComputeGyroContinuityConstraint(const port::SteeringPerceptionMemory& prior_memory,
                                                         const port::ImuSample& imu,
                                                         uint64_t reference_time_ms) {
    GyroContinuityConstraint continuity{};
    continuity.next_state = prior_memory.gyro_continuity;

    const uint64_t prior_valid_ms = prior_memory.gyro_continuity.last_valid_capture_time_ms;
    ...
    if (imu.valid) {
        ...
        const float filtered = 0.7F * imu.gyro_z + 0.3F * prior_memory.gyro_continuity.filtered_yaw_rate;
        continuity.next_state.filtered_yaw_rate = filtered;
        continuity.next_state.heading_delta_deg_150ms =
            ClampHeadingDelta(DecayHeadingDelta(prior_memory.gyro_continuity.heading_delta_deg_150ms, dt_ms) +
        ...
        continuity.next_state.heading_delta_deg_150ms =
            DecayHeadingDelta(prior_memory.gyro_continuity.heading_delta_deg_150ms, elapsed_ms);
