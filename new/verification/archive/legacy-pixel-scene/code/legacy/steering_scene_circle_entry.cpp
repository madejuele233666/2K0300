...
int ContinueCircleEntryStreak(const SteeringSceneContext& context) {
    ...
    if (context.prior_state.scene_debug_candidate == "circle_entry") {
        return std::max(wide.enter_confirm_cycles, context.prior_state.scene_debug_candidate_streak + 1);
    }
...
SteeringSceneOutput BuildCircleEntrySceneOutput(const SteeringSceneContext& context) {
    ...
    output.scene_debug_candidate = "circle_entry";
    output.scene_debug_candidate_streak = ContinueCircleEntryStreak(context);
    output.scene_cross_candidate_score = ComputeCrossSceneScore(context);
