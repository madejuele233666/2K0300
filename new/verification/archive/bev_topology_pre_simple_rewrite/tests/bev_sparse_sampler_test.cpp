...
void TestSparseSamplerClassifiesDrivableAndInvalid() {
    ...
    Expect(samples.layers.size() == 24U, "sparse sampler must expose 24 BEV-uniform layers");
    Expect(std::abs(samples.layers.back().forward_m - 1.500F) < 1e-4F,
           "sparse sampler must cover the configured 1.5m far forward layer");
}
