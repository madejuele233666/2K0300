...
int ReadOptionalIntField(const JsonValue::Object& object,
                         const std::string& key,
                         int fallback,
                         const std::string& context) {
    const JsonValue* value = FindMember(object, key);
    ...
    const double number = RequireNumber(*value, context + "." + key);
...
std::vector<std::string> ReadOptionalStringArrayField(const JsonValue::Object& object,
                                                      const std::string& key,
                                                      const std::string& context) {
    const JsonValue* value = FindMember(object, key);
    if (value == nullptr) {
    ...
    std::vector<std::string> output{};
    JsonValue::Array values = RequireArray(*value, context + "." + key);
    for (std::size_t index = 0; index < values.size(); ++index) {
        output.push_back(RequireString(values[index], context + "." + key + "[" + std::to_string(index) + "]"));
    }
    return output;
...
struct SampleContract {
    ...
    std::string sequence_group{};
    int analysis_cycles = 4;
    ...
    std::vector<std::string> require_scene_phases{};
    std::vector<std::string> forbid_scene_phases{};
    std::vector<std::string> require_debug_candidates{};
    std::vector<std::string> forbid_debug_candidates{};
...
Manifest LoadManifest(const std::filesystem::path& dataset_dir) {
    ...
    for (std::size_t index = 0; index < samples.size(); ++index) {
        ...
        sample.sequence_group =
            ReadOptionalStringField(object, "sequence_group", "", context);
        sample.analysis_cycles =
            std::max(1, ReadOptionalIntField(object, "analysis_cycles", 4, context));
        ...
        sample.require_scene_phases =
            ReadOptionalStringArrayField(object, "require_scene_phases", context);
        sample.forbid_scene_phases =
            ReadOptionalStringArrayField(object, "forbid_scene_phases", context);
        sample.require_debug_candidates =
            ReadOptionalStringArrayField(object, "require_debug_candidates", context);
        sample.forbid_debug_candidates =
            ReadOptionalStringArrayField(object, "forbid_debug_candidates", context);
...
struct SequenceResult {
    ...
    std::vector<std::string> scene_phases{};
    std::vector<std::string> debug_candidates{};
    ls2k::port::LegacySteeringState final_state{};
};

void RememberUnique(std::vector<std::string>& values, const std::string& value) {
    if (value.empty()) {
        return;
    }
...
SequenceResult AnalyzeWithConfirm(const ls2k::port::LegacyCameraFrame& frame,
                                  ...
                                  const ls2k::port::LegacySteeringState& initial_state,
                                  int cycles) {
    ls2k::port::LegacySteeringState state = initial_state;
    SequenceResult sequence{};
    for (int cycle = 0; cycle < std::max(1, cycles); ++cycle) {
        sequence.final =
        ...
        RememberUnique(sequence.scene_phases, sequence.final.perception.scene_phase);
        RememberUnique(sequence.debug_candidates, sequence.final.scene_debug_candidate);
...
bool SawListedValue(const std::vector<std::string>& values, const std::string& expected) {
...
void ValidateOne(const std::filesystem::path& dataset_dir,
                 ...
                 ls2k::port::LegacySteeringState& sequence_state) {
    ...
    const SequenceResult sequence =
        AnalyzeWithConfirm(frame, params, sequence_state, contract.analysis_cycles);
    sequence_state = sequence.final_state;
    ...
    for (const std::string& phase : contract.require_scene_phases) {
        Expect(SawListedValue(sequence.scene_phases, phase), path.string() + " must observe scene_phase " + phase);
    }
    for (const std::string& phase : contract.forbid_scene_phases) {
        Expect(!SawListedValue(sequence.scene_phases, phase), path.string() + " must not observe scene_phase " + phase);
    }
    for (const std::string& candidate : contract.require_debug_candidates) {
        Expect(SawListedValue(sequence.debug_candidates, candidate),
               path.string() + " must observe debug_candidate " + candidate);
    }
    for (const std::string& candidate : contract.forbid_debug_candidates) {
        Expect(!SawListedValue(sequence.debug_candidates, candidate),
               path.string() + " must not observe debug_candidate " + candidate);
    }
...
int main(int argc, char** argv) {
    try {
        ...
        std::map<std::string, ls2k::port::LegacySteeringState> sequence_states{};
        for (const SampleContract& sample : manifest.samples) {
            if (sample.sequence_group.empty()) {
                ls2k::port::LegacySteeringState independent_state{};
                ValidateOne(dataset_dir, sample, manifest, independent_state);
            } else {
                ValidateOne(dataset_dir, sample, manifest, sequence_states[sample.sequence_group]);
            }
        }
