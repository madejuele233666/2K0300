...
void TestCircleProgressionAndDirectionLatch() {
    ...
    const ls2k::legacy::ReferencePolicyResult entry_reference =
        ls2k::legacy::ResolveReferencePolicy(hypotheses, left_circle, second.state, {}, params);
    Expect(entry_reference.reference_path.mode == ls2k::port::ReferenceMode::kArcFollow,
           "circle entry must use arc-follow reference");
    Expect(!PathsMatch(entry_reference.reference_path.sampled_path, hypotheses.ordinary.sampled_path),
           "arc-follow reference must differ from ordinary centerline");
...
void TestZebraHoldAndConstraintsUseTopologyPath() {
    ...
    Expect(analysis.perception.active_module == "zebra",
           "runtime zebra scene must come from topology evidence");
    Expect(analysis.reference_path.mode == ls2k::port::ReferenceMode::kHoldLast,
           "runtime zebra scene must hold through reference policy");
    Expect(analysis.control_constraints.primary_reason == "zebra_hold",
           "runtime zebra behavior must be expressed through constraints");
    Expect(analysis.control_constraints.speed_limit_scale <= params.bev_control_model.min_speed_limit_scale + 1e-6,
           "zebra hold must lower the speed constraint");
}
