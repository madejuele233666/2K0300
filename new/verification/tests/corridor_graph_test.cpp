...
void TestFarOnlyIntervalsNeedControlAnchor() {
    ls2k::port::RuntimeParameters params{};
    ls2k::port::LegacySteeringState prior{};
    ls2k::legacy::CorridorIntervalSet intervals{};
    intervals.valid = true;
    ...
    Expect(!graph.valid, "ordinary graph must require the configured near control anchor");
    Expect(track.fallback_mode == "control_anchor_missing",
           "far-only graph rejection must identify the missing control anchor");
    Expect(!track.valid, "far-only intervals must not form a controllable track");
}
...
void TestShortNearOnlyChainNeedsEvidenceBackedControlHorizon() {
    ...
    ls2k::legacy::CorridorIntervalSet intervals{};
    intervals.valid = true;
    for (std::size_t layer = 1; layer < 5U; ++layer) {
        const float forward = params.bev_topology_sampler.forward_samples_m[layer];
        intervals.layers[layer].forward_m = forward;
...
void TestControlAnchorNeedsNearbySelectedEvidence() {
    ls2k::port::RuntimeParameters params{};
    ls2k::port::LegacySteeringState prior{};
    ls2k::legacy::CorridorIntervalSet intervals{};
    intervals.valid = true;
    ...
    Expect(!graph.valid, "ordinary graph must not back-project distant evidence into the near control anchor");
    Expect(track.fallback_mode == "control_anchor_missing",
           "distant-evidence chain rejection must identify the missing nearby control anchor");
    Expect(!track.valid, "distant-evidence chains must not produce a controllable white path");
}
...
int main() {
    try {
        ...
        TestFarOnlyIntervalsNeedControlAnchor();
        TestShortNearOnlyChainNeedsEvidenceBackedControlHorizon();
        TestControlAnchorNeedsNearbySelectedEvidence();
